...
int ingresoIdProvincia(int* idProvincia, eProvincia provincias[], int tam)
{
	...
	if(idProvincia != NULL && provincias != NULL && tam > 0)
	{
		...
		returnAux = validarProvincia(*idProvincia, provincias, tam);
		while(returnAux == 0)
		{
...
int bajaPersona(ePaciente lista[], int tam, eProvincia provincias[], int tamProv)
{
	...
	if(lista != NULL && tam >0)
	{
		mostrarPacientes(lista, tam, provincias, tamProv);
		printf("  *** Baja Persona *** \n\n");
		...
		else
		{
			...
			confirma = input_ClosedQuestion("Confirma la baja?[s/n] " , "Valor invalido. ");
			if(confirma == 's')
			{
